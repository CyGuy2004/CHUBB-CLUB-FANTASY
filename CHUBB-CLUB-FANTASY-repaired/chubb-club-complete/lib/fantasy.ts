...
export async function playerMetrics(scoring:Scoring='ppr'){
  ...
  let rows:PlayerMetric[]=players.map(player=>{
    ...
    return{player,projection,seasonPoints,average,last,value,rank:0,positionRank:0,source:espnProjection>0?'ESPN Fantasy':fallbackProjection>0?'Sleeper projection':'Projection unavailable',projectionSource:espnProjection>0?'ESPN PROJ':fallbackProjection>0?'Sleeper projection':'Unavailable',opponent,home,gameDate:g?.date,gameStatus:g?.status,matchupRank,matchupLabel:meta.label,matchupTone:meta.tone};
